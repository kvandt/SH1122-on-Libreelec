...
class sh1122(greyscale_device):
    ...
    def _init_sequence(self):
        for cmd in (
            self._const.DISPLAYOFF,  # off
            self._const.SET_DISP_CLK_DIV, 0x50,
            self._const.SET_MUX_RATIO, 0x3F,
            self._const.SET_DISP_OFFSET, 0x00,
            self._const.SET_ROW_ADR, 0x00,
            self._const.SET_DISP_START_LINE | 32,
            self._const.SET_DISCHARGE_LEVEL | 0x00,
            self._const.SET_CTRL_DCDC, 0x80,
            self._const.SET_SEG_REMAP | 0x01,
            self._const.SET_COM_OUT_DIR | 0x08,  # scan from COM0 to COM[N]
            self._const.SET_CONTRAST, 0x80,  # median
            self._const.SET_PRECHARGE, 0x28,
            self._const.SET_VCOM_DESEL, 0x30,
            self._const.SET_VSEG_LEVEL, 0x1E,
            self._const.SET_NORM_INV,  # not inverted
            self._const.SET_COL_ADR_MSB,  # horizontal
            self._const.SET_COL_ADR_LSB,
            self._const.SET_ENTIRE_ON, 0x00, # output follows RAM contents
        ):  
            self.command(cmd)
